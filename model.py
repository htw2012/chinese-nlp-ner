...
from data_utils import get_logger, load_word2vec, calculate_accuracy
...
class Model(object):
    ...
    def __init__(self, name, word_to_id, id_to_tag, parameters):
        ...
        if self.params.feature_dim:
            self.features = tf.placeholder(dtype=tf.float32,
                                           shape=[None, self.params.word_max_len, self.params.feature_dim],
                                           name="Features")
    ...
    def bilstm_layer(self, inputs):
        ...
        with tf.variable_scope("BiLSTM"):
            fw_cell = rnn.LSTMCell(self.params.word_hidden_dim,
                                        use_peepholes=True,
            ...
            #                             use_peepholes=True,
            #                             initializer=self.initializer())
            bw_cell = rnn.LSTMCell(self.params.word_hidden_dim,
                                        use_peepholes=True,
                                        initializer=self.initializer())
    ...
    def project_layer(self, lstm_features, tag_num):
        ...
        with tf.variable_scope("Project", initializer=self.initializer()):
            w1 = tf.get_variable('W1', [self.params.word_hidden_dim * 2, tag_num],
                regularizer=tf.contrib.layers.l2_regularizer(0.001))
            b1 = tf.get_variable('b1', [tag_num])
    ...
    def loss_layer(self, scores, tag_num):
        ...
        with tf.variable_scope("CRF"):
            trans = tf.get_variable('trans', shape=[tag_num, tag_num],
                                    initializer=self.initializer())
    ...
    def create_feed_dict(self, is_train, **kwargs):
        ...
        feed_dict = {
            ...
            self.dropout: 1.0,# do not using dropout
        }

        # 加入外在特征
        if self.params.feature_dim:
            feed_dict[self.features] = kwargs["features"]
    ...
    def predict(self, sess, data):
        ...
        for batch in data.iter_batch():
            ...
            for i in range(len(batch["str_lines"])): # bug fix
                result = []

                for char, gold, pred in zip(str_lines[i][:lengths[i]],
                                            tags[i][:lengths[i]],
                                            batch_paths[i][:lengths[i]]):
                    cur_word = " ".join([char, self.tags[int(gold)], self.tags[int(pred)]])
                    result.append(cur_word)
                results.append([result, end_of_doc[i]]) # cornell格式
        return results

    def predict2(self, sess, data):
        ...
        for batch in data.iter_batch():
            ...
            result = []
            for i in range(len(batch["str_lines"])):

                for char, pred in zip(str_lines[i][:lengths[i]], batch_paths[i][:lengths[i]]):
                    cur_word = " ".join([char, self.tags[int(pred)]])
                    result.append(cur_word)
    ...
    def predict_probility(self, sess, data):
        ...
        for batch in data.iter_batch():
            ...
            for i in range(len(batch["str_lines"])):
                idx = 0
                for char, pred in zip(str_lines[i][:lengths[i]], batch_paths[i][:lengths[i]]): # 愿串+输出结果
                    scs = 1/(1+np.exp(-scores[i][idx]))
                    scs = scs/sum(scs)
                    sc = str(scs[int(pred)])
                    cur_word = " ".join([char, self.tags[int(pred)], sc])
                    result.append(cur_word)
                    idx += 1
                # print("result", result)
        return result      

    def predict3(self, sess, batchManager):
        results = []
        trans = self.trans.eval()

        for batch in batchManager.iter_batch():# 一个batch

            # tags = batch["tags"]
            lengths = batch["len"] # 实际每批的长度
            str_lines = batch["str_lines"] # 实际每批的原始内容
            print "current batch size", len(str_lines)

            scores = self.run_step(sess, False, batch)

            batch_decode_paths = self.decode(scores, lengths, trans)

            for i in range(len(str_lines)):
                result = []

                cur_line = str_lines[i]
                cur_len = lengths[i]
                cur_decode_path = batch_decode_paths[i]# 若干个标签

                for char, pred in zip(cur_line[:cur_len], cur_decode_path[:cur_len]):
                    if pred == None:
                        print "current sentence error", cur_line
                    cur_tag = self.tags[int(pred)]
                    cur_word_tag_pair = [char, cur_tag]
                    result.append(cur_word_tag_pair)
                # if u"重庆舰" in "".join(cur_line):
                #     print "XXX重庆舰", result
                results.append([cur_line, result])
